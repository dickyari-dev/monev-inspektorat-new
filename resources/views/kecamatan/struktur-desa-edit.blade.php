...
<div class="container">
    <div class="card">
        <div class="card-body">
            <form action="{{ route('struktur-desa.update', $data->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6">
                        <table class="table table-borderless">
                            <tr>
                                ...
                                <td>
                                    <select name="id_kecamatan" id="id_kecamatan" class="form-control" required
                                        ...
                                        <option value="{{ $kec->id }}" {{ $kec->id == $data->id_kecamatan ? 'selected' :
                                            '' }}>
                                            {{ $kec->nama_kecamatan }}
                                        </option>
                                        @endforeach
                            ...
                            <tr>
                                ...
                                <td><input type="text" name="nama_pegawai" id="nama_pegawai" class="form-control"
                                        required value="{{ $data->nama_pegawai }}"></td>
                            </tr>

                            <tr>
                                <td><label for="alamat">Alamat Lengkap :</label></td>
                                <td><textarea name="alamat" id="alamat" cols="30" rows="3"
                                        class="form-control">{{ $data->alamat }}</textarea></td>
                            </tr>

                            <tr>
                                <td><label for="telephone">Telephone :</label></td>
                                <td><input type="text" name="telephone" id="telephone" class="form-control" required
                                        value="{{ $data->telephone }}"></td>
                            </tr>
